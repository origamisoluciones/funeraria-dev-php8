<?php
    /**
     * Comprueba si la cadena tiene formato de teléfono
     * 
     * @param string $phone Phone
     * @return bool
     */
    function checkPhone($phone){
        return preg_match('/^\+?([0-9]{1,5})?([0-9]{3})([0-9]{3})([0-9]{3})$/', $phone) == 1 ? true : false;
    }
    
    function checkEmail($email){
        // return preg_match('/^(?:\w+\.?\+?)*\w+@(?:\w+\-)*(?:\w+\.)+\w+$/', $email) == 1 ? true : false;
