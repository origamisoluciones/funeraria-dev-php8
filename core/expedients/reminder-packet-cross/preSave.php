            ...
            foreach(scandir($files) as $file){
                if(preg_match("/{$stamp}--------image.*/", $file)){
                    if(file_exists("{$_SESSION['basePath']}/resources/files/{$_SESSION['company']}/tmp/$expedient/$file")){
                        ...
                        copy("{$_SESSION['basePath']}/resources/files/{$_SESSION['company']}/tmp/$expedient/$file", "{$_SESSION['basePath']}/resources/files/{$_SESSION['company']}/expedients/$expedient/reminder-packet-cross/$type/$model/img/$newFilename");
