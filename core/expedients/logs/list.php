<?php
    ...
    */
    ...
    // DB table to use
    $table = array('Logs', 'Users');
    
    // Table's primary key
    $primaryKey = 'Logs.logID';
    ...
    // indexes
    $columns = array(
        array( 'db' => 'Logs.date', 'dt' => 0,),
        array( 'db' => 'Users.username', 'dt' => 1 ),
        array( 'db' => 'Logs.action', 'dt' => 2 ),
        array( 'db' => 'Logs.description', 'dt' => 3 )
    );
    ...
    // Where condition
    $where = array("Logs.user = Users.userID AND expedient = " . $_GET['expedient']);
    ...
    */
    ...
    $logs = new Logs;
    $logs->createExpedient("Expedients", $_GET['expedient'], "Expendientes - Logs - Consulta", "'Ha consultado el registro del expediente'");
?>