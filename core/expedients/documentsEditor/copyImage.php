<?php
    ...
    if(isset($_POST) && isset($_POST['expedient']) && isset($_POST['imageName']) && isset($_POST['doc']) && isset($_POST['stamp'])){
        ...
        $folder = "{$_SESSION['basePath']}/resources/files/{$_SESSION['company']}/expedients/$expedient/documentsEditor/$doc/img";
        if(!is_dir($folder)){
        ...
        if(!is_dir($_SESSION['basePath'] . "resources/files/{$_SESSION['company']}/tmp/expedientsDocumentEditor/$expedient/$doc")){
            ...
            $file = fopen($_SESSION['basePath'] . "resources/files/{$_SESSION['company']}/tmp/expedientsDocumentEditor/$expedient/$doc/.htaccess", 'w');
            fwrite($file, ' <FilesMatch ".*">
                                Order Allow,Deny
                                Allow from All
                            </FilesMatch>');
        ...
        $filesTmp = scandir("{$_SESSION['basePath']}resources/files/{$_SESSION['company']}/tmp/expedientsDocumentEditor/$expedient/$doc");
        foreach($filesTmp as $file){
            if(preg_match("/{$stamp}--------image.*/", $file)){
                if(file_exists("{$_SESSION['basePath']}resources/files/{$_SESSION['company']}/tmp/expedientsDocumentEditor/$expedient/$doc/$file")){
                    $name = explode('--------', $file)[1];
                    $name = explode('.', $name)[0];
                    $id = $name;
        ...
        if(is_file("$imagesDir/$imageName")){
            ...
            copy("$imagesDir/$imageName", $_SESSION['basePath'] . "resources/files/{$_SESSION['company']}/tmp/expedientsDocumentEditor/$expedient/$doc/{$stamp}--------image_$cont.$extension");
