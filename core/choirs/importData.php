<?php
    ...
    if($extension == 'csv'){
        ...
        if(($fileManager = fopen($file['tmp_name'], "r")) !== FALSE){
            ...
            $logs = new Logs;
            $logs->createSimple("Configuración", "Coros - Importar datos", "'Ha importado los datos de los coros'");
            ...
            while(($line = fgetcsv($fileManager, 1000, ",")) !== FALSE){
                ...
                if($line[0] != ''){
                    ...
                    if(!isset($data[0]) || !isset($data[1]) || !isset($data[2]) || !isset($data[3]) || !isset($data[4]) || !isset($data[5]) || !isset($data[6]) || !isset($data[7]) || !isset($data[8]) || !isset($data[9]) || !isset($data[10])){
                        array_push($errors, "Número de columnas incorrecto. Las columnas son: ID, Nombre, NIF, Dirección, Provincia, Localidad, Código postal, Email, Teléfono, Teléfono Móvil, Otro Teléfono");
                        break;
                    }else{
                        if($i != 0){
                            
                            //Reemplazamos los campos que vengan con comillas para que no rompa la consulta
                            $choirID = str_replace ('"', "",$data[0]);
                            ...
                            $homePhone = str_replace ('"', "",$data[8]);
                            $mobilePhone = str_replace ('"', "",$data[9]);
                            $otherPhone = str_replace ('"', "",$data[10]);

                            //Evaluamos si los campos vienen con un caracter sin codear ('�') (excel los guarda sin codificación) y en ese caso los codeamos
                            if (!preg_match('!!u', $choirID)){
                                $choirID = utf8_encode($choirID);
                            }
                            ...
                            if($data["choirID"] == ""){
                                $response = $choirs->createImport($data);
                                if($response !== true){
                                    $line = $i + 1;
                                    array_push($errors, "Error en la línea $line - $response");
                                }
                            }else{
                                if($choirs->isDelete($data["choirID"])){
                                    $response = $choirs->createImport($data);
                                    if($response !== true){
                                        $line = $i + 1;
                                        array_push($errors, "Error en la línea $line - $response");
                                    ...
                                    $response = $choirs->updateImport($data);
                                    if($response !== true){
                                        $line = $i + 1;
                                        array_push($errors, "Error en la línea $line - $response");
                                    }
