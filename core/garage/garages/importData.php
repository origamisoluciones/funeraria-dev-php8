<?php
    ...
    if($extension == 'csv'){
        ...
        if(($fileManager = fopen($file['tmp_name'], "r")) !== FALSE){
            ...
            $logs = new Logs;
            $logs->createSimple("Configuración", "Talleres - Importar datos", "'Ha importado los datos de los talleres'");
                ...
                if($line[0] != ''){
                    ...
                    if(!isset($data[0]) || !isset($data[1]) || !isset($data[2]) || !isset($data[3]) || !isset($data[4]) || !isset($data[5]) || !isset($data[6]) || !isset($data[7]) || !isset($data[8])){
                        ...
                        if($i != 0){
                            ...
                            $phone = str_replace ('"', "",$data[7]);
                            $supplier = str_replace ('"', "",$data[8]);
                            ...
                            if (!preg_match('!!u', $phone)){
                                $phone = utf8_encode($phone);
                            }

                            if (!preg_match('!!u', $supplier)){
                                $supplier = utf8_encode($supplier);
                            }
                            ...
                            if($supplier != null && $supplier != ''){
                                $supplierID = $suppliers->searchByNameImport($supplier);
                            }
                            ...
                            if($data["ID"] == ""){
                                $response = $garages->createImport($data);
                                if($response !== true){
                                    $line = $i + 1;
                                    array_push($errors, "Error en la línea $line - $response");
                                }
                            }else{
                                if($garages->isDelete($data["ID"])){
                                    $response = $garages->createImport($data);
                                    if($response !== true){
                                        $line = $i + 1;
                                        array_push($errors, "Error en la línea $line - $response");
                                    ...
                                    $response = $garages->updateImport($data);
                                    if($response !== true){
                                        $line = $i + 1;
                                        array_push($errors, "Error en la línea $line - $response");
                                    }
